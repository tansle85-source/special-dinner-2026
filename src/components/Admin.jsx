...
const Admin = () => {
  ...
  const handleNextDraw = async () => {
    ...
    try {
      ...
      const sessionPrizes = (Array.isArray(prizes) ? prizes : []).filter(p => p.session === selectedSession).sort((a,b) => b.rank - a.rank);
            ...
            <div className="module-grid" style={{ display:'flex', flexDirection:'column', gap:'2rem' }}>
              ...
              <div className="card shadow-card">
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem', flexWrap:'wrap', gap:'1rem' }}>
                  <div>
                    <h3 style={{ margin:0 }}>🎁 Published Results ({(Array.isArray(employees) ? employees.filter(e => e.won_prize) : []).length} winners)</h3>
