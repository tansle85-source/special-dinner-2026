window.onerror = function(msg, url, line) { let div = document.createElement('div'); div.style.color='red'; div.style.position='fixed'; div.style.top='0'; div.style.zIndex='9999'; div.innerText = '[JS ERROR] ' + msg + ' (line ' + line + ')'; document.body.appendChild(div); };

// --- CRITICAL INITIALIZATION & UNLOCK ---

/**
 * Company Appreciation Night 2026 - Client App
 * Version: v1.6.88 (STABLE) - PATCHED
 * Last Updated: 2026-04-26
 * Description: High-concurrency optimized event management system
 * 
 * Changelog:
...
console.log("🚀 System v1.6.120 INITIALIZING...");
...
function switchView(view, pushState = true) {
    ...
    document.querySelectorAll('.nav-btn, .mobile-tab-btn, .sidebar-btn').forEach(btn => btn.classList.remove('active'));
    // Hide ALL views
...
function handleUrlRoute() {
    console.log("[Navigation] Routing path:", window.location.pathname);
    var path = window.location.pathname;
    
    // Core bypass: If on root or home, and event is live, go to identify
    var isLive = new Date() >= new Date("2026-04-24T18:00:00");
    
    if (path.includes('/admin')) {
        switchView('admin', false);
    } else if (path.includes('/seating') || path.includes('/identify')) {
        switchView('seating', false);
    } else {
        if (isLive) {
            console.log("[Navigation] Event is LIVE. Forcing Identify view.");
            switchView('identify', false);
        } else {
            switchView('home', false);
        }
    }
...
async function startProfileLockHeartbeat(empId) {
    if (profileLockHeartbeat) clearInterval(profileLockHeartbeat);
    profileLockHeartbeat = setInterval(async () => {
...
async function lockProfile(empId) {
    try {
        var res = await fetch((API_BASE) + "/api/profile/lock", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ empId, deviceId: DEVICE_ID })
        });
        if (!res.ok) {
...
async function unlockProfile(empId) {
    stopProfileLockHeartbeat();
    try {
        await fetch((API_BASE) + "/api/profile/unlock", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ empId, deviceId: DEVICE_ID })
        });
...
function diagLog(msg, color = 'white') {
...
function startReservationTimer() {
    ...
    reservationTimeLeft = 10 * 60; // 10 minutes
...
window.openDoorGiftModal = function(empId) {
    try {
        ...
        var emp = appData.employees.find(e => String(e.id) === String(empId));
        if (!emp) {
        ...
        if (!emp.checked_in) {
            showToast("⚠️ Please check in " + (emp.name) + " first!", "danger");
        ...
        if (appData.tables) {
            for (var t = 0; t < appData.tables.length; t++) {
                var seat = appData.tables[t].find(s => s && String(s.empId) === String(empId));
        ...
        if (doorGiftModal) {
            ...
            setTimeout(() => { if (doorGiftPinInput) doorGiftPinInput.focus(); }, 150);
        } else {
...
window.revokeDoorGift = async function(empId) {
    var pin = prompt("Enter committee PIN to revoke this door gift claim:");
    if (!pin) return;

    try {
        var res = await fetch((API_BASE) + "/api/door-gift/revoke", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.closeDoorGiftModal = async function() {
...
window.claimDoorGiftFast = async function(empId) {
    if (!empId) return;
    try {
        var res = await fetch((API_BASE) + "/api/checkin/combined", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.deletePrize = async function(prizeId) {
    if (confirm("Are you sure you want to delete this prize? Any drawn winners for it will be lost.")) {
        try {
            var res = await fetch((API_BASE) + "/api/prize/delete", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
var listLayout, btnToggleView, modal, closeModal, resForm, modalTitle, modalSeatInfo, modalEmpName;
...
var adminTabBtnDirectory = document.getElementById('sidebar-btn-directory');


var adminTabBtnVisual = document.getElementById('sidebar-btn-visual');
...
var adminTabBtns = [
    ...
    document.getElementById('sidebar-btn-directory'),
    document.getElementById('sidebar-btn-visual'),
    ...
    document.getElementById('admin-tab-btn-entertainment'),
    document.getElementById('admin-tab-btn-feedback-results'),
...
var adminTabViews = [
    ...
    document.getElementById('admin-tab-entertainment'),
    document.getElementById('admin-tab-feedback-results'),
...
if (adminTabBtnDoorgifts) adminTabBtns.push(adminTabBtnDoorgifts);
if (adminTabDoorgifts) adminTabViews.push(adminTabDoorgifts);
if (adminTabBtnFeedbackManager) adminTabBtns.push(adminTabBtnFeedbackManager);
if (adminTabFeedbackManager) adminTabViews.push(adminTabFeedbackManager);
...
async function updateSetting(key, value) {
    try {
        var res = await fetch((API_BASE) + "/api/settings/update", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function setupPaxAutocomplete() {
    ...
    inputs.forEach(input => {
        ...
        input.addEventListener('input', () => {
            ...
            var matches = appData.employees.filter(emp => {
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.pax-search-container')) {
            document.querySelectorAll('.pax-search-results').forEach(el => el.classList.add('hidden'));
        }
    });
}
...
async function loadData(force = false) {
    ...
    try {
        var res = await fetch((API_BASE) + "/api/data");
        if (!res.ok) {
            var errorText = await res.text().catch(() => "No error body");
...
// Deprecated 鈥?backend now handles persistence via API
// function saveData() { }
...
function setupEventListeners() {
    ...
    // Admin Tabs
    if (adminTabBtnDashboard) adminTabBtnDashboard.addEventListener('click', async () => { showAdminTab(adminTabDashboard, adminTabBtnDashboard); renderAdminDashboard(); });
    if (adminTabBtnDirectory) adminTabBtnDirectory.addEventListener('click', async () => { showAdminTab(adminTabDirectory, adminTabBtnDirectory); renderAdminDirectory(); });
    if (adminTabBtnVisual) adminTabBtnVisual.addEventListener('click', async () => { showAdminTab(adminTabVisual, adminTabBtnVisual); renderVisualAdminMap(); });
    if (adminTabBtnDoorgifts) adminTabBtnDoorgifts.addEventListener('click', async () => { showAdminTab(adminTabDoorgifts, adminTabBtnDoorgifts); renderDoorGiftMgmt(); });
    if (adminTabBtnSettings) adminTabBtnSettings.addEventListener('click', async () => { showAdminTab(adminTabSettings, adminTabBtnSettings); renderSettings(); });
    if (adminTabBtnPrizes) adminTabBtnPrizes.addEventListener('click', async () => { showAdminTab(adminTabPrizes, adminTabBtnPrizes); renderAdminDashboard(); });
    if (adminTabBtnLuckydraw) adminTabBtnLuckydraw.addEventListener('click', async () => { showAdminTab(adminTabLuckydraw, adminTabBtnLuckydraw); renderLuckyDrawWinners(); });
    if (adminTabBtnClaims) adminTabBtnClaims.addEventListener('click', async () => { showAdminTab(adminTabClaims, adminTabBtnClaims); renderWinnerClaims(); });

    if (navSeating) navSeating.addEventListener('click', async () => switchView('seating'));
    if (navCheckin) navCheckin.addEventListener('click', async () => switchView('checkin'));
    if (navLuckydraw) navLuckydraw.addEventListener('click', async () => switchView('luckydraw'));
    if (navEntertainment) navEntertainment.addEventListener('click', async () => switchView('entertainment'));
    if (navHome) navHome.addEventListener('click', async () => switchView('home'));

    if (navAdmin) navAdmin.addEventListener('click', async () => {
        if (isAdminLoggedIn) {
            switchView('admin');
        } else {
            adminLoginForm.reset();
    ...
    if (closeAdminLoginModal) {
        closeAdminLoginModal.addEventListener('click', async () => {
    ...
    if (elCloseAdminEmpModal) {
        elCloseAdminEmpModal.addEventListener('click', async () => {
    ...
    if (elCloseAdminEditEmpModal) {
        elCloseAdminEditEmpModal.addEventListener('click', async () => {
    ...
    if (elCloseEditSeatModal) {
        elCloseEditSeatModal.addEventListener('click', async () => {
    ...
    if (elCloseCreateAdminModal) {
        elCloseCreateAdminModal.addEventListener('click', async () => {
    ...
    if (elCloseEditAdminModal) {
        elCloseEditAdminModal.addEventListener('click', async () => {
    ...
    if (elCloseAdminBatchModal) {
        elCloseAdminBatchModal.addEventListener('click', async () => {
    ...
    if (elCloseDoorGiftModal) {
        elCloseDoorGiftModal.addEventListener('click', async () => {
    ...
    if (btnTestConnection) {
        btnTestConnection.addEventListener('click', async () => {
            ...
            btnTestConnection.innerHTML = "⌛Testing...";
    ...
    document.getElementById('edit-admin-role')?.addEventListener('change', (e) => {
        var section = document.getElementById('edit-admin-permissions-section');
        if (section) section.style.display = e.target.value.includes('Super Admin') ? 'none' : 'block';
    });

    // Toggle All Permissions Logic
    var toggleAll = (allSelector, itemSelector) => {
    ...
    if (createAdminForm) {
        createAdminForm.addEventListener('submit', async (e) => {
            ...
            var perms = Array.from(document.querySelectorAll('#new-admin-permissions-section input[name="permissions"]:checked')).map(cb => cb.value);
            try {
                var res = await fetch((API_BASE) + "/api/admin/add", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    publicSessionBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            publicSessionBtns.forEach(b => b.classList.remove('active'));
    ...
    if (createPrizeForm) {
        createPrizeForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            ...
            try {
                var res = await fetch((API_BASE) + "/api/prize/add", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id, session, name, quantity: qty, prize_rank: rank })
                });
                if (!res.ok) { var d = await res.json(); alert(d.error); return; }
    ...
    if (editPrizeForm) {
        editPrizeForm.addEventListener('submit', async (e) => {
            ...
            try {
                var res = await fetch((API_BASE) + "/api/prize/update", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id, session, name, quantity: qty, prize_rank: rank })
                });
    ...
    if (btnResetLuckyDraw) {
        btnResetLuckyDraw.addEventListener('click', async () => {
            if (confirm("Are you sure you want to reset ALL lucky draw winners for all sessions?")) {
                try {
                    await fetch((API_BASE) + "/api/luckydraw/reset", { method: 'POST' });
                    await loadData();
                    renderAdminDashboard();
    ...
    window.addEventListener('beforeunload', () => {
        ...
        if (currentSelectedSeat) {
            ...
            // No direct async call in beforeunload
        }
    });
    ...
    if (btnBatchClearAll) {
        btnBatchClearAll.addEventListener('click', async () => {
            if (confirm("Clear all selections in this table?")) {
                document.querySelectorAll('#admin-batch-table-body input[type="checkbox"]').forEach(cb => cb.checked = false);
            }
        });
    }
    ...
    if (btnToggleManual) {
        btnToggleManual.addEventListener('click', async () => {
    ...
    if (btnCloseManual) {
        btnCloseManual.addEventListener('click', async () => {
    ...
    if (navGift) navGift.addEventListener('click', async () => switchView('doorgift'));
...
async function handleAdminResetEmployees() {
    var confirm1 = confirm("⚠️ CRITICAL ACTION: Are you sure you want to reset the employee database?\n\nThis will ERASE all employees, seat bookings, check-ins, and winners.");
    if (!confirm1) return;
    ...
    try {
        ...
        var res = await fetch((API_BASE) + "/api/admin/employees/reset", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        var data = await res.json();
        if (res.ok) {
            alert("鉁?Employee database has been reset.");
            window.location.reload();
        } else {
            alert("鉂?Reset failed: " + data.error);
        }
    } catch (err) {
        console.error("Reset error:", err);
        alert("鉂?Network error during reset.");
    }
}
...
function onDoorGiftScanSuccess(decodedText) {
    ...
    var emp = appData.employees.find(e => String(e.id) === String(decodedText) || e.email === decodedText);
    if (!emp) {
...
function renderDoorGiftDetails(emp) {
    ...
    if (emp.door_gift_claimed) {
        statusBox.innerText = "⚠️ PROHIBITED: GIFT ALREADY CLAIMED";
        statusBox.style.background = "rgba(255, 71, 87, 0.1)";
        statusBox.style.color = "var(--danger)";
        confirmBtn.disabled = true;
        confirmBtn.style.opacity = "0.5";
    } else {
        statusBox.innerText = "READY TO CLAIM 馃巵";
...
async function handleConfirmDoorgift() {
    if (!currentGiftUser) return;
    ...
    try {
        var res = await fetch((API_BASE) + "/api/door-gift/claim", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    if (pullIndicator) {
        ...
        window.addEventListener('touchend', async (e) => {
            ...
            if (finalPullDistance >= PULL_THRESHOLD && window.scrollY === 0) await triggerGlobalRefresh();
            else {
...
async function triggerGlobalRefresh() {
    ...
    if (btnSaveEventTimer) {
        btnSaveEventTimer.addEventListener('click', async () => {
    ...
    if (btnSaveDoorGiftPin && doorGiftPinSettingInput) {
        btnSaveDoorGiftPin.addEventListener('click', async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    var errorData = await res.json().catch(() => ({}));
...
function updateViewModeUI() {
    if (!btnToggleView || !floorLayout || !listLayout) return;

    if (isListView) {
        floorLayout.classList.add('hidden');
        listLayout.classList.remove('hidden');
        btnToggleView.querySelector('.text').innerText = 'Switch to Map View';
        btnToggleView.querySelector('.icon').innerText = 'MAP';
    } else {
        floorLayout.classList.remove('hidden');
        listLayout.classList.add('hidden');
        btnToggleView.querySelector('.text').innerText = 'Switch to List View';
        btnToggleView.querySelector('.icon').innerText = 'LIST';
    }
}
...
function setupSSE() {
    ...
    eventSource.onmessage = async (event) => {
        ...
        if (msg.ts) {
            ...
            if (guestLat) {
                guestLat.innerText = "鈥?Live: " + (latency) + "ms";
                guestLat.style.display = 'inline';
                // Briefly highlight or stay visible
                setTimeout(() => { guestLat.style.opacity = '0.5'; }, 2000);
            }
        }
        ...
        if (msg.type === 'profile_lock_update') {
            ...
            if (!viewIdentify.classList.contains('hidden')) {
                var results = appData.employees.filter(e => {
        ...
        if (msg.type === 'seating_update') {
            ...
            if (action === 'table_status') {
                ...
                var ts = appData.tablesStatus.find(t => t.tableIdx === tableIdx);
...
function updateSeatElement(el, seat, tableIdx, seatIdx, isAdmin) {
    ...
    if (seat) {
        ...
        el.onclick = isAdmin ?
            ...
            async () => await openReservationModal(tableIdx, seatIdx);
    }
}
...
function renderTables() {
    ...
    appData.tables.forEach((table, tableIdx) => {
        var tStatusObj = appData.tablesStatus ? appData.tablesStatus.find(ts => ts.tableIdx === tableIdx) : null;
        ...
        var bookedSeats = table.filter(s => s !== null).length;
        ...
        if (true) { 
            if (isOffline) {
                ...
                tableCenter.addEventListener('click', async () => showToast("⚠️ Table " + (tableIdx + 1) + " is reserved for offline booking.", "danger"));
            } else {
        ...
        if (tLock) {
            ...
            var booker = appData.employees.find(e => e.id === tLock.empId);
        ...
        table.forEach((seat, seatIdx) => {
            ...
            if (seat) {
                ...
                if (isOffline) {
                    ...
                    seatEl.addEventListener('click', async () => await openReservationModal(tableIdx, seatIdx));
                } else {
...
function renderTableList() {
    ...
    appData.tables.forEach((table, tableIdx) => {
        ...
        var availableSeats = table.filter(s => s === null).length;
        ...
        if (appData.tableLocks) {
            Object.keys(appData.tableLocks).forEach(key => {
            ...
            else { reserveBtn.innerText = "RESERVE ENTIRE TABLE"; reserveBtn.onclick = async () => await openReservationModal(tableIdx, null); }
            ...
            table.forEach((seat, seatIdx) => {
                ...
                if (seat) {
                    ...
                    seatBtn.addEventListener('click', async () => await openReservationModal(tableIdx, seatIdx));
                }
...
function performIdentifySearch() {
    ...
    var employeeResults = appData.employees.filter(emp =>
...
window.selectBookingProfile = async function (empId) {
    var success = await lockProfile(empId);
    if (!success) return;

    currentBookingEmployee = appData.employees.find(e => e.id === empId);
    if (currentBookingEmployee) {
...
async function openReservationModal(tableIdx, seatIdx) {
    ...
    fetch((API_BASE) + "/api/table/unlock", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ empId: currentBookingEmployee.id })
    ...
    // PERMISSIVE BYPASS: If the lock belongs to ME, just let me in.
    var myLockKey = Object.keys(appData.tableLocks || {}).find(key => {
        var lock = appData.tableLocks[key];
    ...
    if (myLockKey) {
        ...
        // We let it pass through instead of showing the toast.
    } else {
    ...
    if (isOffline && !isAdminLoggedIn) {
        showToast("⚠️ Table " + (tableIdx + 1) + " is reserved for offline booking.", "danger");
        return;
    }
    ...
    if (!isViewOnly) {
        try {
            var res = await fetch((API_BASE) + "/api/table/lock", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ tableIdx, seatIdx: (seatIdx === null ? -1 : seatIdx), empId: currentBookingEmployee.id })
            });
            if (!res.ok) {
                var data = await res.json();
                showToast("⚠️ " + (data.error || 'Booking in Progress'), "danger");
                return;
            }
        } catch (err) {
    ...
    paxSearchContainers.forEach((container, idx) => {
...
async function handleReservationSubmit(e) {
    ...
    try {
        ...
        var body = isTable ? {
            ...
            paxNames: paxNames,
            paxEmpIds: paxEmpIds,
            paxDiets: paxEmpIds.map(id => {
                if (!id) return 'Normal';
                var found = appData.employees.find(e => e.id === id);
        ...
        if (!res.ok) {
            ...
            if (res.status === 409) {
                showToast("⚠️ " + (data.error || 'Table already taken!'), "danger");
            } else {
...
async function unlockTable() {
    ...
    try {
        await fetch((API_BASE) + "/api/table/unlock", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tableIdx, seatIdx, empId: currentBookingEmployee.id })
        });
    } catch (err) {
...
async function cancelTableBooking() {
    if (!currentBookingEmployee) return;
    ...
    if (isLikelyTableBooking) {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/table/cancel", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ tableIdx: primaryTableIdx, empId: currentBookingEmployee.id })
            });
            if (res.ok) {
        ...
        try {
            ...
            for (var seat of mySeats) {
                await fetch((API_BASE) + "/api/seat/cancel-own", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ tableIdx: seat.tIdx, seatIdx: seat.sIdx, empId: currentBookingEmployee.id })
                });
            }
...
window.checkInUser = async function (empId) {
    if (empId) {
        try {
...
async function submitDoorGiftClaim(e) {
    ...
    try {
        ...
        showToast("Thank you, hope you enjoy the night! 馃巵", "success");
        await loadData();
...
window.removeReservation = async function (tIdx, sIdx) {
    if (confirm("Are you sure you want to remove this reservation?")) {
        try {
            await fetch((API_BASE) + "/api/seat/remove", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
window.openEditSeatModal = function (tIdx, sIdx, empId, empName) {
    ...
    var emp = appData.employees.find(e => e.id === empId);
    ...
    var updateDisplay = (eId, eName) => {
        ...
        var found = appData.employees.find(e => e.id === eId);
...
async function handleAdminEditSeatSubmit(e) {
    e.preventDefault();
    ...
    try {
        ...
        var moveRes = await fetch((API_BASE) + "/api/seat/move", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!moveRes.ok) {
             ...
             var res = await fetch((API_BASE) + "/api/seat/reserve", {
                 method: 'POST',
                 headers: { 'Content-Type': 'application/json' },
...
async function handleAdminBatchSubmit(e) {
    e.preventDefault();
    ...
    try {
        var res = await fetch((API_BASE) + "/api/admin/table/assign", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.deleteEmployee = async function (empId) {
    if (confirm("Are you sure you want to delete employee " + (empId) + "?")) {
        try {
            var res = await fetch((API_BASE) + "/api/employee/delete", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
window.deleteAdmin = async function (username) {
    if (confirm("Are you sure you want to delete admin account '" + (username) + "' ? ")) {
        try {
            var res = await fetch((API_BASE) + "/api/admin/delete", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function handleEditEmployeeSubmit(e) {
    ...
    fetch((API_BASE) + "/api/employee/update", {
        ...
        if (!res.ok) {
            var d = await res.json().catch(() => ({ error: 'Unknown server error' }));
...
function handleCsvUpload(e) {
    ...
    reader.onload = async function (evt) {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/employee/import", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function initFilters() {
    ...
    var departments = [...new Set(appData.employees.map(e => e.dept))].sort();
...
function renderAdminDirectory() {
    ...
    filteredData.forEach(({ emp, seatInfo }) => {
        var tr = document.createElement('tr');
        ...
        if (seatInfo) {
            ...
            checkinActionHtml = emp.checked_in ?
                "<button class=\"admin-action-btn btn-compact-outline\" onclick=\"updateCheckin('" + (emp.id) + "', 'no')\" title=\"Undo Check-In\">鈫╋笍</button>" :
                "<button class=\"admin-action-btn btn-compact-primary\" onclick=\"updateCheckin('" + (emp.id) + "', 'yes')\" title=\"Check In\">鉁?/button>";

            managementButtonsHtml = "\r\n            <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditProfileModal('" + (emp.id) + "')\" title=\"Edit Profile\">馃懁</button>\r\n                <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditSeatModal(" + (seatInfo.t - 1) + ", " + (seatInfo.s - 1) + ", '" + (emp.id) + "', '" + (safeEmpName) + "')\" title=\"Change Seat\">馃獞</button>\r\n                <button class=\"admin-action-btn btn-compact-danger\" onclick=\"removeReservation(" + (seatInfo.t - 1) + ", " + (seatInfo.s - 1) + ")\" title=\"Unassign Seat\">馃毇</button>\r\n        ";
        } else {
            ...
            managementButtonsHtml = "\r\n            <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditProfileModal('" + (emp.id) + "')\" title=\"Edit Profile\">馃懁</button>\r\n                <button class=\"admin-action-btn btn-compact-primary\" onclick=\"openEditSeatModal(null, null, '" + (emp.id) + "', '" + (safeEmpName) + "')\" title=\"Assign Seat\">馃捄+</button>\r\n                <button class=\"admin-action-btn btn-compact-danger\" onclick=\"deleteEmployee('" + (emp.id) + "')\" title=\"Delete Employee\">馃棏锔?/button>\r\n        ";
        }
        ...
        tr.innerHTML = "\r\n            <td data-label=\"Location\" class=\"font-medium\">" + (seatInfo ? 'T' + seatInfo.t + ' - S' + seatInfo.s : '<span style="color:var(--text-muted)">-</span>') + "</td>\r\n            <td data-label=\"Employee Name\"><strong class=\"font-bold\">" + (emp.name) + "</strong></td>\r\n            <td data-label=\"ID\"><code>" + (emp.id) + "</code></td>\r\n            <td data-label=\"Email\" style=\"font-size:0.85rem; max-width: 150px; overflow: hidden; text-overflow: ellipsis;\">" + (emp.email || '<span style="color:var(--text-muted)">-</span>') + "</td>\r\n            <td data-label=\"Dept\"><span style=\"font-size:0.85rem;\">" + (emp.dept) + "</span></td>\r\n            <td data-label=\"Diet\"><span style=\"font-size:0.85rem;\" class=\"diet-badge " + (emp.diet === 'vegetarian' ? 'veg' : 'non-veg') + "\">" + (emp.diet || '-') + "</span></td>\r\n            <td data-label=\"Gift\" class=\"text-center\">\r\n                " + (emp.door_gift_claimed === 1 ? 
                    ...
                    (emp.checked_in ? "<button class=\"admin-action-btn btn-compact-primary\" onclick=\"window.openDoorGiftModal('" + (safeEmpId) + "')\" title=\"Claim Gift\">馃巵</button>" : '<span style="color:var(--text-muted); font-size: 0.7rem;">Wait Check-In</span>')
...
window.updateCheckin = async function (empId, status) {
    try {
        var undo = status === 'no';
        var res = await fetch((API_BASE) + "/api/seat/checkin", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function renderDoorGiftMgmt() {
    ...
    var filtered = appData.employees.filter(emp => {
    ...
    filtered.forEach(emp => {
        ...
        row.innerHTML = "\r\n            <td>\r\n                <div style=\"font-weight:700; color:var(--primary-dark);\">" + (emp.name) + "</div>\r\n                <div style=\"font-size:0.75rem; color:var(--text-muted);\">ID: " + (emp.id) + "</div>\r\n            </td>\r\n            <td>" + (emp.diet || 'Standard') + "</td>\r\n            <td>" + (emp.checked_in ? '<span class="status-badge status-checked-in">Checked In</span>' : '<span class="status-badge status-pending">Pending</span>') + "</td>\r\n            <td>\r\n                " + (isClaimed 
                    ...
                    ? "<button class=\"btn-door-gift\" style=\"padding: 0.4rem 0.8rem; font-size: 0.75rem;\" onclick=\"window.claimDoorGiftFast('" + (emp.id) + "')\">馃巵 Claim</button>" 
                    : (isClaimed 
                        ? "<button class=\"btn-danger\" style=\"padding: 0.4rem 0.8rem; font-size: 0.75rem; background:#dc3545; border:none; border-radius:4px; color:white;\" onclick=\"window.revokeDoorGift('" + (emp.id) + "')\">馃攧 Revoke</button>" 
                        : '')) + "\r\n            </td>\r\n        ";
...
function showAdminTab(targetView, targetBtn) {
    ...
    adminTabBtns.forEach(b => { if (b) b.classList.remove('active'); });
...
function renderVisualAdminMap() {
    ...
    appData.tables.forEach((table, tableIdx) => {
        ...
        var bookedSeatsCount = table.filter(s => s !== null).length;
        ...
        table.forEach((seat, seatIdx) => {
            ...
            if (seat) {
                ...
                if (adminSeatingMode === 'seat') {
                    seatEl.style.cursor = 'pointer';
                    seatEl.addEventListener('click', async () => openEditSeatModal(tableIdx, seatIdx, seat.empId, seat.name));
                } else {
                    seatEl.style.cursor = 'default';
                ...
                if (adminSeatingMode === 'seat') {
                    seatEl.style.cursor = 'pointer';
                    seatEl.addEventListener('click', async () => openEditSeatModal(tableIdx, seatIdx, null, 'Available'));
                } else {
                    seatEl.style.cursor = 'default';
...
function updateActivePrizeIndicator() {
    ...
    var currentPrizes = (appData.prizes || []).filter(p => p.session === currentLuckydrawSession);
    
    var nextPrize = null;
    
    // 1. Check if we have a "sticky" prize from a recent redraw
    if (stickyPrizeId) {
        nextPrize = currentPrizes.find(p => p.id === stickyPrizeId && (p.drawnIds || []).length < p.quantity);
    }
    
    // 2. Otherwise find the first available prize by rank/name
    if (!nextPrize) {
        nextPrize = currentPrizes
            .filter(p => (p.drawnIds || []).length < p.quantity)
            .sort((a, b) => {
...
window.openEditPrizeModal = function (prizeId) {
    ...
    try {
        ...
        if (!modal) return;
        var prize = appData.prizes.find(p => String(p.id) === String(prizeId));
        if (!prize) return;
...
window.revokeWinner = async function(prizeId, empId) {
    if (!confirm('Are you sure you want to revoke this winner?')) return;
    try {
        var res = await fetch(`${API_BASE}/api/luckydraw/revoke`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prizeId, empId })
        });
        if (res.ok) { await loadData(); renderLuckyDrawWinners(); showToast('Winner revoked.'); }
    } catch (err) { alert(err.message); }
};

// Removed duplicate performLuckyDraw definition


function displayWinner(data) {
    const { winner, prizeName } = data;
    winnerDisplay.innerHTML = "<h2>Winner: " + winner.name + "</h2><h3>Prize: " + prizeName + "</h3>";
    loadData().then(() => renderLuckyDrawWinners());
}
...
window.toggleFullScreen = function() {
    ...
    if (!document.fullscreenElement) {
        ...
        btnToggleFullscreen.textContent = '鉁?Exit Fullscreen';
    } else {
        ...
        btnToggleFullscreen.textContent = '鉀?Fullscreen';
    }
}
...
window.toggleTableStatus = async function (tableIdx, newStatus) {
    try {
        var res = await fetch((API_BASE) + "/api/table/toggle-status", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
unassignBtn.addEventListener('click', async () => {
    ...
    if (confirm("Are you sure you want to unassign " + (currentEditingSeat.empName) + " from Table " + (currentEditingSeat.tIdx + 1) + "?")) {
        try {
            var res = await fetch((API_BASE) + "/api/seat/remove", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
window.openEditAdminModal = function (username) {
    ...
    var admin = appData.admins.find(a => a.username === username);
    if (!admin) return;
...
editAdminForm.addEventListener('submit', async (e) => {
    ...
    var perms = Array.from(document.querySelectorAll('#edit-admin-permissions-section input[name="edit-permissions"]:checked')).map(cb => cb.value);

    try {
        var res = await fetch((API_BASE) + "/api/admin/update", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function enforcePermissions() {
    ...
    if (currentTab && permissionMap[currentTab] && !hasPermission(permissionMap[currentTab])) {
        var firstVisible = adminTabBtns.find(b => b && b.style.display !== 'none');
...
function updateCountdown() {
    ...
    if (!eventTimeStr) {
        console.warn('[Countdown] No event_timer found in settings. Remaining at default.');
        return;
    }
    ...
    try {
        ...
        // If it's still NaN, try to massage the string
        if (isNaN(targetDate)) {
            var massaged = eventTimeStr.replace(' ', 'T');
    ...
    if (isNaN(targetDate)) {
        console.error('[Countdown] Invalid target date received from server:', eventTimeStr);
        // Show a diagnostic hint if set to 00
        var timerLabel = document.querySelector('.pull-label');
        if (timerLabel && timerLabel.innerText.includes('Pull')) {
            console.log('[Countdown] Timer data invalid. Please set a valid date in Admin Settings.');
        }
        return;
    }
    ...
    console.log("[Countdown] Target: " + (eventTimeStr) + ", TimeLeft: " + (timeLeft) + "ms");

    var timerGrid = document.getElementById('countdown-timer');
    var endedMsg = document.getElementById('event-ended-message');

    if (timeLeft <= 0) {
        console.log('[Countdown] Event has already started or ended.');
...
function renderSettings() {
    document.querySelectorAll('.toggle-feature').forEach(btn => {
        ...
        if (key === 'maintenance_mode') {
            ...
            // ON = Green (Success), OFF = Red (Danger)
            btn.style.background = val === 'on' ? 'var(--success)' : 'var(--danger)';
        }
        btn.style.color = 'white';
    ...
    if (doorGiftPinSettingInput) doorGiftPinSettingInput.value = doorGiftPin;
}

function updateOverlay(viewId, settingKey, message) {
    var view = document.getElementById(viewId);
    if (!view) return;
    var isActive = (getSettingValue(settingKey) === 'on');
    var overlay = view.querySelector('.feature-lock-overlay');
    if (!isActive) {
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.className = 'feature-lock-overlay';
            overlay.innerHTML = "<div class=\"lock-content\"><i class=\"fas fa-lock\"></i><p>" + (message) + "</p></div>";
...
async function renderDynamicFeedbackForm() {
    ...
    if (!container) return;
    try {
        var res = await fetch((API_BASE) + "/api/feedback/questions");
...
if (closeFeedbackModal) {
    closeFeedbackModal.addEventListener('click', async () => feedbackModal.classList.add('hidden'));
}
...
async function initQRScanner(targetId = "qr-reader", customCallback = null) {
    ...
    setTimeout(async () => {
        for (var attempt of searchAttempts) {
            try {
                ...
                await html5QrScanner.start(constraints, config, customCallback || onScanSuccess, onScanError);
        ...
        try {
            ...
            if (devices && devices.length > 0) {
                diagLog("Found " + (devices.length) + " cameras. Labels: " + (devices.map(d=>d.label).join(", ")), "yellow");
            } else {
...
window.forceRestartCamera = async function() {
...
function onScanSuccess(decodedText, decodedResult) {
    ...
    var emp = appData.employees.find(e => String(e.id).trim() === text);
...
function showScannedUserDetails(emp) {
    ...
    if (emp.checked_in) {
        ...
        btn.innerHTML = 'CHECK IN <span class="kbd-hint">Press Enter 鈫?/span>';
        btn.disabled = false;
        btn.style.opacity = "1";
    }
...
async function handleConfirmCheckin() {
    ...
    try {
        ...
        if (res.ok) {
            var data = await res.json();
            showToast("鉁?" + (emp.name) + " Checked-In & Gift Claimed!", "success");
...
// --- QR GENERATION LOGIC ---
async function drawBadgeOnCanvas(emp, canvas, tempQrDiv) {
...
// Initial initialization delay to prevent QRCode.js race conditions in loops (v1.5.66)
    ...
    var qrImg = await new Promise((resolve) => {
        var attempts = 0;
        var check = () => {
...
window.downloadSingleQR = async function(empId) {
    var emp = appData.employees.find(e => String(e.id) === String(empId));
    if (!emp) {
    ...
    var success = await drawBadgeOnCanvas(emp, canvas, tempDiv);
...
function renderLuckyDrawWinners() {
    ...
    var updateSummary = (sumElem, qtyElem) => {
        if (!sumElem) return;
        if (drawsCount > 0) {
    ...
    currentPrizes.forEach(prize => {
        (prize.drawnIds || []).forEach(empId => {
            ...
            if (emp) {
                ...
                if (searchTerm) {
                    var keywords = searchTerm.split(/\s+/).filter(k => k.length > 0);
                ...
                filteredCount++;
                var winItem = (appData.winners || []).find(w => w.prizeId === prize.id && w.empId === emp.id);
...
window.performLuckyDraw = async function (count = 1) {
    ...
    var availablePrizes = currentPrizes.filter(p => (p.drawnIds || []).length < p.quantity);
    ...
    try {
        var res = await fetch(`${API_BASE}/api/luckydraw/roll`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (count === 1) {
            ...
            var allCurrentlyEligible = appData.employees.filter(emp => emp.checked_in && !winnersSet.has(emp.id));
            ...
            if (allCurrentlyEligible.length <= 100) {
                ...
                var others = allCurrentlyEligible.filter(emp => emp.id !== winner.winner.id);
                var sampleSize = 99;
                var sample = [];
...
window.toggleClaimStatus = async function (prizeId, empId, isClaimed) {
    try {
        var res = await fetch((API_BASE) + "/api/luckydraw/claim", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.resetLuckyDrawSession = async function () {
    ...
    try {
        var res = await fetch((API_BASE) + "/api/luckydraw/reset-session", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function renderWinnerClaims() {
    ...
    var winnersList = (appData.winners || []).map(w => {
        var prize = (appData.prizes || []).find(p => p.id === w.prizeId);
        var emp = (appData.employees || []).find(e => e.id === w.empId);
        return { ...w, prize, emp };
    }).filter(item => {
...
if (feedbackForm) {
    feedbackForm.addEventListener('submit', async (e) => {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/feedback/submit", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
if (closeVotingModal) {
    closeVotingModal.addEventListener('click', async () => votingModal.classList.add('hidden'));
}

async function openVotingModal(category) {
    try {
        var res = await fetch((API_BASE) + "/api/voting/candidates");
        var allCandidates = await res.json();
        var candidates = allCandidates.filter(c => c.category === category);
        ...
        candidates.forEach(c => {
            ...
            btn.onclick = async () => submitVote(c.id, category);
...
async function submitVote(candidateId, category) {
    ...
    try {
        ...
        var res = await fetch((API_BASE) + "/api/voting/vote", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function renderEntertainment() {
    ...
    if (showPerf) {
        try {
            var response = await fetch((API_BASE) + "/api/voting/candidates");
            var candidates = await response.json();
            var activePerf = candidates.find(c => c.category === 'performance' && c.is_open);
    ...
    if (btnNominate) btnNominate.onclick = async () => document.getElementById('nomination-modal').classList.remove('hidden');
    if (btnVoteMale) btnVoteMale.onclick = async () => openVotingModal('best_dress_male');
    if (btnVoteFemale) btnVoteFemale.onclick = async () => openVotingModal('best_dress_female');
    feedbackCard.querySelector('button').onclick = async () => {
...
async function renderVotingMgmt() {
    try {
        var response = await fetch((API_BASE) + "/api/voting/candidates");
        var candidates = await response.json();
        var resultsResp = await fetch((API_BASE) + "/api/voting/results");
        var results = await resultsResp.json();
        ...
        candidates.forEach(c => {
            var result = results.find(r => r.id === c.id) || { vote_count: 0, total_score: 0 };
            var rowHtml = '';
...
async function toggleVotingCandidate(id, isOpen) {
    try {
        var res = await fetch((API_BASE) + "/api/admin/voting/candidates/toggle", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function openAddCandidateModal(category) {
    ...
    if (name) {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/admin/voting/candidates/add", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function renderFeedbackMgmt() {
    try {
        var response = await fetch((API_BASE) + "/api/feedback/questions");
...
async function renderFeedbackResults() {
    try {
        var res = await fetch((API_BASE) + "/api/admin/feedback/stats");
...
// Export buttons
document.getElementById('btn-export-voting')?.addEventListener('click', async () => {
    window.location.href = '/api/admin/export/voting';
});

document.getElementById('btn-export-feedback-dynamic')?.addEventListener('click', async () => {
...
if (closeNominationBtn) {
    closeNominationBtn.onclick = async () => nominationModal.classList.add('hidden');
}
...
if (nominateSearchInput) {
    nominateSearchInput.addEventListener('input', async (e) => {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/employees/search?q=" + encodeURIComponent(query));
...
if (formNominate) {
    formNominate.addEventListener('submit', async (e) => {
        ...
        try {
            var res = await fetch((API_BASE) + "/api/nominations/submit", {
                method: 'POST',
                body: formData
            });
...
async function loadAdminNominations() {
    try {
        var res = await fetch((API_BASE) + "/api/nominations/list");
        var noms = await res.json();
        ...
        var maleNoms = noms.filter(n => n.category === 'male').sort((a,b) => b.ai_score - a.ai_score);
        var femaleNoms = noms.filter(n => n.category === 'female').sort((a,b) => b.ai_score - a.ai_score);
        
        var renderGrid = (list, container) => {
            container.innerHTML = '';
...
async function triggerAiRanking() {
    ...
    btn.innerHTML = '閳?AI is thinking...';
    btn.disabled = true;
    
    try {
        var res = await fetch((API_BASE) + "/api/nominations/ai-rank", { method: 'POST' });
        var data = await res.json();
...
async function promoteAiTop3() {
    ...
    var ogText = btn.innerHTML;
    btn.innerHTML = '閳?Promoting...';
    btn.disabled = true;
    try {
        var res = await fetch((API_BASE) + "/api/admin/nominations/promote", { method: 'POST' });
        var data = await res.json();
...
window.showWinnerReveal = async function(category) {
    try {
        ...
        // Filter and sort
        var categoryCandidates = candidates.filter(c => c.category === category);
        var candidatesWithScores = categoryCandidates.map(c => {
            var r = results.find(res => res.id === c.id) || { vote_count: 0, total_score: 0 };
            return {
                ...c,
...
async function handlePrizeCsvUpload(e) {
    ...
    reader.onload = async function (evt) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (parts.length >= 4) {
                const session = parseInt(parts[0].trim());
        ...
        try {
            const res = await fetch((API_BASE) + '/api/prize/import', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function checkToggleOverlays() {
    ...
    var dressNomOn = getSettingValue('feature_bestdress_nominations') === 'on';
...
async function init() {
    ...
    viewLuckydraw = document.getElementById('view-luckydraw');
    viewEntertainment = document.getElementById('view-entertainment');
    viewAdmin = document.getElementById('view-admin');
    viewHome = document.getElementById('view-home');
    navSeating = document.getElementById('nav-seating');
    navCheckin = document.getElementById('nav-checkin');
    navLuckydraw = document.getElementById('nav-luckydraw');
    navEntertainment = document.getElementById('nav-entertainment');
    navAdmin = document.getElementById('nav-admin');
    navHome = document.getElementById('nav-home');

    console.log("[Resilience] Initializing Application v1.6.120 (STABLE)...");
    ...
    var versionBadge = document.getElementById('debug-version-badge');
    if (versionBadge) versionBadge.textContent = "v1.6.120 (STABLE)";
...
window.redrawLastWinner = async function() {
    var winners = (appData.prizes || []).flatMap(p => (p.drawnIds || []).map(id => ({ prizeId: p.id, empId: id, ts: 0 })));
    if (winners.length === 0) {
    ...
    var currentSessionPrizes = (appData.prizes || []).filter(p => p.session === currentLuckydrawSession);
    var lastWinner = null;
...
// FORCE REDIRECT
setTimeout(() => { if (typeof switchView === 'function') switchView('identify', false); }, 500);
