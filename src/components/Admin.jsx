      ...
      <main className="modern-content">
        ...
        <div className="scroll-container">
          ...
          {activeModule === 'guest-feedback' && (() => {
            const [fbStatus, setFbStatus] = React.useState('CLOSED');
            const [fbQuestions, setFbQuestions] = React.useState([]);
            const [fbResponses, setFbResponses] = React.useState({ questions: [], total: 0 });
            const [fbTab, setFbTab] = React.useState('questions');
            const [newQ, setNewQ] = React.useState({ text: '', type: 'text' });
            ...
            const clearResponses = async () => {
              ...
              setFbResponses({ questions: fbResponses.questions.map(q => ({...q, answers:[]})), total: 0 });
            };
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
                ...
                <div className="card shadow-card" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem' }}>
                  ...
                  <div style={{ display:'flex', gap:'0.75rem', alignItems:'center' }}>
                    <div style={{ display:'flex', borderRadius:'10px', overflow:'hidden', border:'1.5px solid #e2e8f0' }}>
                      {['questions','responses'].map(t => (
                        <button key={t} onClick={() => setFbTab(t)}
                          style={{ padding:'0.5rem 1rem', border:'none', fontWeight:700, fontSize:'0.82rem', cursor:'pointer', textTransform:'capitalize',
                    ...
                    <button onClick={toggleStatus}
                      style={{ padding:'0.5rem 1.1rem', borderRadius:'10px', border:'none', fontWeight:800, fontSize:'0.82rem', cursor:'pointer',
                ...
                {fbTab === 'questions' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                    {/* Add new question */}
                    <div className="card shadow-card">
                      <h4 style={{ margin:'0 0 1rem', color:'#1D1D1D', fontWeight:800 }}>➕ Add Question</h4>
                      <div style={{ display:'flex', gap:'0.75rem', flexWrap:'wrap' }}>
                        <input
                          ...
                          style={{ flex:1, minWidth:'200px', padding:'0.75rem 1rem', borderRadius:'10px', border:'1.5px solid #e2e8f0', fontSize:'0.92rem', fontFamily:'Outfit,sans-serif', outline:'none' }}
                        />
                        <select value={newQ.type} onChange={e => setNewQ(q => ({...q, type: e.target.value}))}
                          style={{ padding:'0.75rem', borderRadius:'10px', border:'1.5px solid #e2e8f0', fontFamily:'Outfit,sans-serif', fontWeight:700, color:'#1D1D1D', outline:'none' }}>
                          <option value="text">📝 Text Answer</option>
                          <option value="rating">⭐ Star Rating (1–5)</option>
                        </select>
                        <button onClick={addQuestion}
                          style={{ padding:'0.75rem 1.4rem', background:'#0A8276', color:'white', border:'none', borderRadius:'10px', fontWeight:800, cursor:'pointer', fontSize:'0.9rem', fontFamily:'Outfit,sans-serif' }}>
                          Add
                        </button>
                      </div>
                    </div>
                    ...
                    {/* Question list */}
                    <div className="card shadow-card">
                      <h4 style={{ margin:'0 0 1rem', color:'#1D1D1D', fontWeight:800 }}>📋 Questions ({fbQuestions.length})</h4>
                      {fbQuestions.length === 0
                        ? <p style={{ color:'#94a3b8', textAlign:'center', padding:'2rem' }}>No questions yet. Add one above.</p>
                        : fbQuestions.map((q, i) => (
                          <div key={q.id} style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.85rem', background:'#f8fafc', borderRadius:'12px', marginBottom:'0.5rem', border:'1px solid #e2e8f0' }}>
                            <span style={{ color:'#94a3b8', fontWeight:700, fontSize:'0.8rem', minWidth:'20px' }}>Q{i+1}</span>
                            {editingQ?.id === q.id ? (
                              <>
                                <input value={editingQ.question_text} onChange={e => setEditingQ(x => ({...x, question_text: e.target.value}))}
                                  style={{ flex:1, padding:'0.5rem', borderRadius:'8px', border:'1.5px solid #0A8276', fontFamily:'Outfit,sans-serif', fontSize:'0.9rem', outline:'none' }} />
                                <select value={editingQ.type} onChange={e => setEditingQ(x => ({...x, type: e.target.value}))}
                                  style={{ padding:'0.5rem', borderRadius:'8px', border:'1.5px solid #e2e8f0', fontFamily:'Outfit,sans-serif', fontWeight:700 }}>
                                  <option value="text">📝 Text</option>
                                  <option value="rating">⭐ Rating</option>
                                </select>
                                <button onClick={saveEdit} style={{ padding:'0.4rem 0.9rem', background:'#0A8276', color:'white', border:'none', borderRadius:'8px', fontWeight:700, cursor:'pointer', fontSize:'0.8rem' }}>Save</button>
                                <button onClick={() => setEditingQ(null)} style={{ padding:'0.4rem 0.9rem', background:'#f1f5f9', border:'none', borderRadius:'8px', fontWeight:700, cursor:'pointer', fontSize:'0.8rem' }}>Cancel</button>
                              </>
                            ) : (
                              <>
                                <span style={{ flex:1, fontWeight:600, fontSize:'0.9rem' }}>{q.question_text}</span>
                                <span style={{ fontSize:'0.72rem', padding:'2px 8px', borderRadius:'99px', background: q.type==='rating' ? 'rgba(251,191,36,0.15)' : 'rgba(99,102,241,0.1)', color: q.type==='rating' ? '#b45309' : '#4f46e5', fontWeight:700 }}>
                                  {q.type === 'rating' ? '⭐ Rating' : '📝 Text'}
                                </span>
                                <button onClick={() => setEditingQ({...q})} style={{ padding:'0.4rem 0.8rem', background:'#f0f9ff', border:'1px solid #bae6fd', borderRadius:'8px', color:'#0369a1', fontWeight:700, cursor:'pointer', fontSize:'0.75rem' }}>Edit</button>
                                <button onClick={() => deleteQ(q.id)} style={{ padding:'0.4rem 0.8rem', background:'#fef2f2', border:'1px solid #fca5a5', borderRadius:'8px', color:'#ef4444', fontWeight:700, cursor:'pointer', fontSize:'0.75rem' }}>Del</button>
                              </>
                            )}
                          </div>
                        ))
                ...
                {fbTab === 'responses' && (
                  <div className="card shadow-card">
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
                      ...
                      <button onClick={clearResponses} style={{ padding:'0.4rem 0.9rem', background:'rgba(239,68,68,0.08)', border:'1px solid #ef4444', borderRadius:'8px', color:'#ef4444', fontWeight:700, cursor:'pointer', fontSize:'0.8rem' }}>
                        🗑 Clear All
                      </button>
                    </div>
                    {fbResponses.questions?.length === 0
                      ? <p style={{ color:'#94a3b8', textAlign:'center', padding:'2rem' }}>No responses yet.</p>
                      : fbResponses.questions?.map((q, i) => (
                        <div key={q.id} style={{ marginBottom:'1.5rem', background:'#f8fafc', borderRadius:'14px', padding:'1rem', border:'1px solid #e2e8f0' }}>
                          <div style={{ fontWeight:800, marginBottom:'0.75rem', color:'#1D1D1D', fontSize:'0.9rem' }}>
                            Q{i+1}: {q.question_text}
                            <span style={{ marginLeft:'0.5rem', fontSize:'0.72rem', padding:'2px 8px', borderRadius:'99px', background:'rgba(10,130,118,0.1)', color:'#0A8276', fontWeight:700 }}>
                          ...
                          {q.type === 'rating' ? (
                              ...
                              {[1,2,3,4,5].map(star => {
                                ...
                                return (
                                  <div key={star} style={{ textAlign:'center' }}>
                                    <div style={{ fontWeight:800, fontSize:'1.1rem' }}>{'⭐'.repeat(star)}</div>
                                    <div style={{ fontWeight:900, fontSize:'1.3rem', color:'#1D1D1D' }}>{count}</div>
                            ...
                            <div style={{ maxHeight:'160px', overflowY:'auto', display:'flex', flexDirection:'column', gap:'0.4rem' }}>
                              {q.answers?.length === 0
                                ...
                                : q.answers?.map((a, j) => (
                                  <div key={j} style={{ background:'white', padding:'0.5rem 0.75rem', borderRadius:'8px', border:'1px solid #e2e8f0', fontSize:'0.85rem', color:'#1e293b' }}>
                                    "{a.answer_text}"
                                  </div>
                                ))
                              }
