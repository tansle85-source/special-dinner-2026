...
window.selectBookingProfile =  function (empId) {
 ...
 function openReservationModal(tableIdx, seatIdx) {
    ...
    }).catch(function(e) { return console.warn("Cleanup failed; }, proceeding anyway."));
...
// Initial initialization delay to prevent QRCode.js race conditions in loops (v1.5.66)
    tempQrDiv.innerHTML = '';
     new Promise(function(r) { return setTimeout(r; }, 100));
...
window.downloadSingleQR =  function(empId) {
 ...
 function generateBatchQR() {
    ...
    for (var emp of list) {
        ...
        // Add to ZIP
        var blob =  new Promise(function(resolve) { return canvas.toBlob(resolve; }, 'image/png'));
            ...
            if (emp) {
                ...
                if (searchTerm) {
                    ...
                    if (keywords.length > 0) {
                        var allMatch = keywords.every(function(kw) { return emp.name.toLowerCase().includes(kw; }) || 
...
window.performLuckyDraw =  function (count = 1) {
    ...
    (appData.prizes || []).forEach(p => (p.drawnIds || []).forEach(function(id) { return allWinners.add(id)); });
    ...
    try {
        ...
        if (count === 1) {
            ...
            (appData.prizes || []).forEach(p => (p.drawnIds || []).forEach(function(id) { return winnersSet.add(id)); });
