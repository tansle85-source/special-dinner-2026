...
function renderTableList() {
    ...
    appData.tables.forEach(function(table, tableIdx) {
        ...
        if (seatingMode === 'table') {
            ...
            var isFullyBooked = table.every(function(s) { return s !== null); };
        ...
        if (appData.tableLocks) {
            ...
            var occupiedByOthers = table.some(function(s) { return s !== null && (currentBookingEmployee ? s.empId !== currentBookingEmployee.id : true)); };
...
window.selectBookingProfile =  function (empId) {
 ...
 function openReservationModal(tableIdx, seatIdx) {
    ...
    if (isTable) {
        ...
        var occupiedByOthers = table.some(function(s) { return s !== null && s.empId !== currentBookingEmployee.id); };
        var isFullyBooked = table.every(function(s) { return s !== null); };
 ...
 function cancelTableBooking() {
    ...
    var hasUnassignedGuests = tableSeats.some(function(s) { return s !== null && s.empId === null); };
    // Determine if they own the primary seat (Seat 0)
    var ownsPrimarySeat = mySeats.some(function(s) { return s.sIdx === 0); };
...
function updateCancelButtonVisibility() {
    ...
    if (currentBookingEmployee) {
        var hasBooking = appData.tables.some(function(table) { return table.some(s => s && s.empId === currentBookingEmployee.id)); };
        if (hasBooking) {
...
window.performLuckyDraw =  function (count = 1) {
    ...
    (appData.prizes || []).forEach(function(p) { return (p.drawnIds || []).forEach(id => allWinners.add(id))); };
    ...
    try {
        ...
        if (count === 1) {
            ...
            (appData.prizes || []).forEach(function(p) { return (p.drawnIds || []).forEach(id => winnersSet.add(id))); };
...
// --- Admin Best Dress Nominations ---
 function loadAdminNominations() {
    try {
        ...
        var hasRanked = noms.some(function(n) { return n.ai_score > 0); };
...
window.showWinnerReveal =  function(category) {
    try {
        ...
        if (document.documentElement.requestFullscreen) {
            document.documentElement.requestFullscreen().catch(function(err) { return console.log('Fullscreen rejected:', err)); };
        }
...
window.redrawLastWinner =  function() {
    var winners = (appData.prizes || []).flatMap(function(p) { return (p.drawnIds || []).map(id => (; }{ prizeId: p.id, empId: id, ts: 0 })));
    if (winners.length === 0) {
