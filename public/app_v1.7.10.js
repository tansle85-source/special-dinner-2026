// --- System v1.7.10 (STABLE-LEGACY) INITIALIZATION ---
/**
 * Company Appreciation Night 2026 - Client App
 * Version: v1.7.10 (STABLE-LEGACY) (ROBUST-INITIALIZATION)
 * Last Updated: 2026-04-27
 * Description: Moved DOM element assignments to init() and fixed seating view visibility
 * 
 * Changelog:
 * - v1.7.10 (STABLE-LEGACY): Fixed countdown hang and removed duplicate logic.
 * - v1.7.10 (STABLE-LEGACY): Distributed PAX locking and focused Admin interface.
 * - v1.7.10 (STABLE-LEGACY): Fixed sidebar tab active state and cleaned up redundant functions.
 * - v1.7.10 (STABLE-LEGACY): Integrated Google Gemini AI for feedback analysis.
 * - v1.7.10 (STABLE-LEGACY): Optimized data load with request coalescing.
 */

console.log("🚀 System v1.7.10 (STABLE-LEGACY) INITIALIZING...");
...
function setupEventListeners() {
    ...
    var btnImportPrizeCsv = document.getElementById('btn-import-prize-csv');
    if (btnImportPrizeCsv && prizeCsvUpload) {
        btnImportPrizeCsv.addEventListener('click', function() {
            prizeCsvUpload.click();
        });
    }
    ...
    if (versionBadge) versionBadge.textContent = "v1.7.10 (STABLE-LEGACY) (FORCE-REFRESH)";
...
async function handleAdminResetEmployees() {
    ...
    try {
        showToast("閳寸爣esetting database...");
...
function renderTables() {
    // Re-verify elements if missing (v1.7.10 (STABLE-LEGACY) Resilience)
        ...
        if (true) { 
            if (isOffline) {
                ...
                tableCenter.addEventListener('click', async () => {
                    if (tLock) {
                        showToast("棣冩晙 This table is currently being locked for booking by another user.", "danger");
                        return;
                    }
...
async function openReservationModal(tableIdx, seatIdx) {
    ...
    // --- INSTANT SWAP v1.7.10 (STABLE-LEGACY) ---
    ...
    if (!isViewOnly) {
        try {
            ...
            if (!res.ok) {
                var data = await res.json();
                showToast("⚠️ " + (data.error || 'Booking in progress. Please choose other table.'), "danger");
                return;
            }
        } catch (err) {
...
async function handleReservationSubmit(e) {
    ...
    try {
        ...
        switchView('home');
        showToast("Success! Your group is now booked at Table " + (tableIdx + 1) + ". 棣冨竴");
...
async function unlockTable() {
    ...
    try {
        var res = await fetch((API_BASE) + "/api/table/unlock", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    filteredData.forEach(({ emp, seatInfo }) => {
        ...
        if (seatInfo) {
            ...
            checkinActionHtml = emp.checked_in ?
                "<button class=\"admin-action-btn btn-compact-outline\" onclick=\"updateCheckin('" + (emp.id) + "', 'no')\" title=\"Undo Check-In\">•愁偀鏅粭?/button>" :
                "<button class=\"admin-action-btn btn-compact-primary\" onclick=\"updateCheckin('" + (emp.id) + "', 'yes')\" title=\"Check In\">•?/button>";

            managementButtonsHtml = "\r\n            <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditProfileModal('" + (emp.id) + "')\" title=\"Edit Profile\">妫ｅ啯鍣?/button>\r\n                <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditSeatModal(" + (seatInfo.t - 1) + ", " + (seatInfo.s - 1) + ", '" + (emp.id) + "', '" + (safeEmpName) + "')\" title=\"Change Seat\">妫ｅ啰宕?/button>\r\n                <button class=\"admin-action-btn btn-compact-danger\" onclick=\"removeReservation(" + (seatInfo.t - 1) + ", " + (seatInfo.s - 1) + ")\" title=\"Unassign Seat\">妫ｅ啯鐦?/button>\r\n        ";
        } else {
            ...
            managementButtonsHtml = "\r\n            <button class=\"admin-action-btn btn-compact-outline\" onclick=\"openEditProfileModal('" + (emp.id) + "')\" title=\"Edit Profile\">妫ｅ啯鍣?/button>\r\n                <button class=\"admin-action-btn btn-compact-primary\" onclick=\"openEditSeatModal(null, null, '" + (emp.id) + "', '" + (safeEmpName) + "')\" title=\"Assign Seat\">妫ｅ啯宕?</button>\r\n                <button class=\"admin-action-btn btn-compact-danger\" onclick=\"deleteEmployee('" + (emp.id) + "')\" title=\"Delete Employee\">妫ｅ啯顥旈柨?/button>\r\n        ";
        }
        ...
        tr.innerHTML = "\r\n            <td data-label=\"Location\" class=\"font-medium\">" + (seatInfo ? 'T' + seatInfo.t + ' - S' + seatInfo.s : '<span style="color:var(--text-muted)">-</span>') + "</td>\r\n            <td data-label=\"Employee Name\"><strong class=\"font-bold\">" + (emp.name) + "</strong></td>\r\n            <td data-label=\"ID\"><code>" + (emp.id) + "</code></td>\r\n            <td data-label=\"Email\" style=\"font-size:0.85rem; max-width: 150px; overflow: hidden; text-overflow: ellipsis;\">" + (emp.email || '<span style="color:var(--text-muted)">-</span>') + "</td>\r\n            <td data-label=\"Dept\"><span style=\"font-size:0.85rem;\">" + (emp.dept) + "</span></td>\r\n            <td data-label=\"Diet\"><span style=\"font-size:0.85rem;\" class=\"diet-badge " + (emp.diet === 'vegetarian' ? 'veg' : 'non-veg') + "\">" + (emp.diet || '-') + "</span></td>\r\n            <td data-label=\"Gift\" class=\"text-center\">\r\n                " + (emp.door_gift_claimed === 1 ? 
                    ...
                    (emp.checked_in ? "<button class=\"admin-action-btn btn-compact-primary\" onclick=\"window.openDoorGiftModal('" + (safeEmpId) + "')\" title=\"Claim Gift\">妫ｅ啫杩?/button>" : '<span style="color:var(--text-muted); font-size: 0.7rem;">Wait Check-In</span>')
...
function renderDoorGiftMgmt() {
    ...
    filtered.forEach(emp => {
        ...
        row.innerHTML = "\r\n            <td>\r\n                <div style=\"font-weight:700; color:var(--primary-dark);\">" + (emp.name) + "</div>\r\n                <div style=\"font-size:0.75rem; color:var(--text-muted);\">ID: " + (emp.id) + "</div>\r\n            </td>\r\n            <td>" + (emp.diet || 'Standard') + "</td>\r\n            <td>" + (emp.checked_in ? '<span class="status-badge status-checked-in">Checked In</span>' : '<span class="status-badge status-pending">Pending</span>') + "</td>\r\n            <td>\r\n                " + (isClaimed 
                    ...
                    ? "<button class=\"btn-door-gift\" style=\"padding: 0.4rem 0.8rem; font-size: 0.75rem;\" onclick=\"window.claimDoorGiftFast('" + (emp.id) + "')\">妫ｅ啫杩?Claim</button>" 
                    : (isClaimed 
                        ? "<button class=\"btn-danger\" style=\"padding: 0.4rem 0.8rem; font-size: 0.75rem; background:#dc3545; border:none; border-radius:4px; color:white;\" onclick=\"window.revokeDoorGift('" + (emp.id) + "')\">妫ｅ啯鏁?Revoke</button>" 
                        : '')) + "\r\n            </td>\r\n        ";
...
window.toggleFullScreen = function() {
    ...
    if (!document.fullscreenElement) {
        ...
        if (typeof btnToggleFullscreen !== 'undefined' && btnToggleFullscreen) btnToggleFullscreen.textContent = '閴?Exit Fullscreen';
    } else {
        ...
        if (typeof btnToggleFullscreen !== 'undefined' && btnToggleFullscreen) btnToggleFullscreen.textContent = '棣冩灱閿?Fullscreen';
    }
};
...
// --- END OF FILE v1.7.10 (STABLE-LEGACY) ---
...
window.performLuckyDraw = async function (count = 1) {
    ...
    try {
        ...
        if (count === 1) {
            ...
            await loadData();
            winnerDisplay.innerHTML = '<h2 style="color: var(--accent-gold);">棣冨竴 ' + count + ' Winners Drawn! 棣冨竴</h2>';
            renderLuckyDrawWinners();
            renderAdminDashboard();
...
function displayWinner(data) {
    ...
    winnerDisplay.innerHTML = 
        '<div style="font-size: 1.5rem; color: var(--accent-gold); margin-bottom: 0.5rem;">棣冨竸 WINNER - ' + rankText + prizeName + ' 棣冨竸</div>' +
        ...
        tr.innerHTML = 
            ...
            '<td data-label="Status" style="text-align: center;">' +
                '<button class="' + (isClaimed ? 'btn-secondary' : 'btn-primary') + '" ' +
                    ...
                    (isClaimed ? '閴?Claimed' : '棣冨返 Mark Claimed') +
                '</button>' +
            '</td>' +
    ...
    try {
        ...
        questions.forEach(q => {
            ...
            if (q.question_type === 'rating') {
                ...
                for (var i = 1; i <= 5; i++) {
                    html += '<span class="star-btn" data-val="' + i + '">閳?/span>';
                }
                html += '</div>';
...
async function renderFeedbackResults() {
    try {
        ...
        var { totalSubmissions, stats } = await res.json();
        ...
        var renderGrid = (list, container) => {
            ...
            list.forEach((nom, index) => {
                ...
                card.innerHTML = 
                    ...
                    (nom.ai_score > 0 ? 
                        ('<div style="margin-top:0.5rem; background: rgba(0,0,0,0.3); padding: 0.2rem; border-radius: 4px;"><span style="color:var(--accent-gold); font-weight:bold;">' + nom.ai_score + ' / 100</span><br><span style="font-size:0.65rem; color:#aaa; display:block; padding:0 0.2rem;" title="' + nom.ai_reasoning + '">棣冩寱 "' + nom.ai_reasoning + '"</span></div>') : 
                        '<div style="margin-top:0.5rem; font-size:0.75rem; color:#888;">棣冩櫟 Unranked</div>');
...
async function triggerAiRanking() {
    ...
    btn.innerHTML = '闂?AI is thinking...';
    btn.disabled = true;
...
async function handlePrizeCsvUpload(e) {
    ...
    reader.onload = async function (evt) {
        ...
        try {
            var res = await fetch((API_BASE) + '/api/prize/import', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function renderPrizeInventory() {
    var body = document.getElementById('admin-prizes-table-body');
    if (!body) return;
    body.innerHTML = '';
    var sortedPrizes = (appData.prizes || []).slice().sort(function(a, b) {
    ...
    sortedPrizes.forEach(function(prize) {
        var drawnCount = (prize.drawnIds || []).length;
        var tr = document.createElement('tr');
        tr.innerHTML = '<td data-label="Session">S' + prize.session + '</td>' +
                     '<td data-label="Rank">' + (prize.prize_rank || '-') + '</td>' +
                     '<td data-label="Name">' + prize.name + '</td>' +
                     '<td data-label="Qty">' + drawnCount + '/' + prize.quantity + '</td>' +
                     '<td data-label="Actions">' +
                         '<div style="display:flex; gap:0.4rem; justify-content: center;">' +
                             '<button class="admin-action-btn btn-compact-primary" onclick="window.openEditPrizeModal(\'' + prize.id + '\')">Edit</button>' +
                             '<button class="admin-action-btn btn-compact-danger" onclick="window.deletePrize(\'' + prize.id + '\')">Del</button>' +
                         '</div>' +
                     '</td>';
        body.appendChild(tr);
    });
    ...
    if (entView) {
        ...
        if (!perfOn && !dressVoteOn && !dressNomOn && !feedOn) {
            if (!overlay) {
                overlay = document.createElement('div');
                overlay.className = 'feature-closed-overlay';
                overlay.innerHTML = '<div class="feature-closed-content"><span class="lock-icon">棣冩晙</span><h3>Features Closed</h3><p>All entertainment features are currently closed.</p></div>';
...
async function init() {
    console.log("[Resilience] Initializing Application v1.7.10 (STABLE-LEGACY) (FIX-READY)...");
    ...
    var versionBadge = document.getElementById('debug-version-badge');
    if (versionBadge) versionBadge.textContent = "v1.7.10 (STABLE-LEGACY) (STABLE)";
