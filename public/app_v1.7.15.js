...
function setupEventListeners() {
    ...
    document.getElementById('new-admin-role')?.addEventListener('change', function(e) {
    ...
    document.getElementById('edit-admin-role')?.addEventListener('change', function(e) {
    ...
    publicSessionBtns.forEach(function(btn) {
        btn.addEventListener('click', function(e) {
    ...
    if (createPrizeForm) {
        createPrizeForm.addEventListener('submit', async function(e) {
            e.preventDefault();
    ...
    if (editPrizeForm) {
        editPrizeForm.addEventListener('submit', async function(e) {
            e.preventDefault();
    ...
    if (btnResetLuckyDraw) {
        btnResetLuckyDraw.addEventListener('click', async function() {
    ...
    if (identifySearchInput) {
        ...
        identifySearchInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                clearTimeout(searchTimeout);
...
async function submitDoorGiftClaim(e) {
    ...
    try {
        ...
        console.error("Door gift claim error:", err);
    }
}
...
// ==========================================
 ...
 function triggerGlobalRefresh() {
    ...
    window.setAdminInteractionMode = function(mode) {
        ...
        var tableBtn = document.getElementById('admin-table-mode-btn');
        if (seatBtn) seatBtn.classList.toggle('active', mode === 'seat');
        if (tableBtn) tableBtn.classList.toggle('active', mode === 'table');
...
function renderTables() {
    ...
    appData.tables.forEach(function(table, tableIdx) {
        ...
        table.forEach(function(seat, seatIdx) {
...
function performSearch() {
    ...
    appData.employees.forEach(function(emp) {
        ...
        if (
            ...
            var tableIdx = null;
            var seatIdx = null;
            appData.tables.forEach(function(table, tIdx) {
                table.forEach(function(seat, sIdx) {
...
async function cancelTableBooking() {
    ...
    if (!confirm("Are you sure you want to cancel your current booking?")) return;
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Try own-cancel bypass if regular cancel fails
...
async function handleReservationSubmit(e) {
 ...
 function openReservationModal(tableIdx, seatIdx) {
    ...
    paxSearchContainers.forEach(function(container, idx) {
...
function handleCsvUpload(e) {
    ...
    if (btnImportPrizeCsv && prizeCsvUpload) {
        console.log("[PrizeCSV] Attaching listener");
        btnImportPrizeCsv.onclick = function() { 
            console.log("[PrizeCSV] Clicked");
...
// Initial initialization delay to prevent QRCode.js race conditions in loops (v1.5.66)
    ...
    var qrImg =  new Promise(function(resolve) {
        var attempts = 0;
        var check = function() {
...
function renderLuckyDrawWinners() {
    ...
    var currentPrizes = (appData.prizes || []).filter(function(p) { return p.session === currentLuckydrawSession; });
    currentPrizes.sort(function(a, b) {
        var rankA = parseInt(a.prize_rank) || 0;
        var rankB = parseInt(b.prize_rank) || 0;
        if (rankA !== rankB) return rankB - rankA;
        return a.name.localeCompare(b.name);
...
window.performLuckyDraw =  function (count = 1) {
    ...
    try {
        ...
        if (count === 1) {
            ...
            var winnerIndex = wheelCandidates.findIndex(function(c) { return c.id === winner.winner.id; });
            ...
            function animate(timestamp) {
                ...
                wheelCandidates.forEach(function(cand, i) {
...
if (adminBatchForm) {
    adminBatchForm.addEventListener('submit',  function(e) {
        e.preventDefault();
...
if (nominateSearchInput) {
    nominateSearchInput.addEventListener('input',  function(e) {
...
if (formNominate) {
    formNominate.addEventListener('submit',  function(e) {
        e.preventDefault();
 ...
 function loadAdminNominations() {
    try {
        ...
        var renderGrid = function(list, container) {
            ...
            list.forEach(function(nom, index) {
