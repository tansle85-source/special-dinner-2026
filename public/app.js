...
window.onerror = function(message, source, lineno, colno, error) {
    ...
    if (!message.includes('ResizeObserver')) {
        alert("閳跨媴绗?System Error Detected!\n" + message + " at line " + lineno);
    }
    return false;
};
...
window.openDoorGiftModal = function(empId) {
    try {
        ...
        if (!emp.checked_in) {
            showToast("閳跨媴绗?Please check in " + (emp.name) + " first!", "danger");
...
// Deprecated 闁?backend now handles persistence via API
// function saveData() { }
...
function setupEventListeners() {
    ...
    if (btnTestConnection) {
        btnTestConnection.addEventListener('click', async () => {
            ...
            btnTestConnection.innerHTML = "閳辨笨esting...";
...
async function handleAdminResetEmployees() {
    var confirm1 = confirm("閳跨媴绗?CRITICAL ACTION: Are you sure you want to reset the employee database?\n\nThis will ERASE all employees, seat bookings, check-ins, and winners.");
    if (!confirm1) return;
    ...
    try {
        ...
        var data = await res.json();
        if (res.ok) {
            alert("闁?Employee database has been reset.");
            window.location.reload();
        } else {
            alert("闁?Reset failed: " + data.error);
        }
    } catch (err) {
        console.error("Reset error:", err);
        alert("闁?Network error during reset.");
    }
}
...
function setupSSE() {
    ...
    eventSource.onmessage = async (event) => {
        ...
        if (msg.ts) {
            ...
            if (guestLat) {
                guestLat.innerText = "闁?Live: " + (latency) + "ms";
    ...
    appData.tables.forEach((table, tableIdx) => {
        ...
        if (true) { 
            if (isOffline) {
                ...
                tableCenter.addEventListener('click', async () => showToast("閳跨媴绗?Table " + (tableIdx + 1) + " is reserved for offline booking.", "danger"));
            } else {
...
async function openReservationModal(tableIdx, seatIdx) {
    ...
    if (isOffline && !isAdminLoggedIn) {
        showToast("閳跨媴绗?Table " + (tableIdx + 1) + " is reserved for offline booking.", "danger");
        return;
    }
...
async function handleReservationSubmit(e) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (res.status === 409) {
                showToast("閳跨媴绗?" + (data.error || 'Table already taken!'), "danger");
            } else {
    ...
    filteredData.forEach(({ emp, seatInfo }) => {
        ...
        if (seatInfo) {
            ...
            checkinActionHtml = emp.checked_in ?
                "<button class=\"admin-action-btn btn-compact-outline\" onclick=\"updateCheckin('" + (emp.id) + "', 'no')\" title=\"Undo Check-In\">闁愁偀鏅粭?/button>" :
                "<button class=\"admin-action-btn btn-compact-primary\" onclick=\"updateCheckin('" + (emp.id) + "', 'yes')\" title=\"Check In\">闁?/button>";
...
function showScannedUserDetails(emp) {
    ...
    if (emp.checked_in) {
        ...
        btn.innerHTML = 'CHECK IN <span class="kbd-hint">Press Enter 闁?/span>';
        btn.disabled = false;
        btn.style.opacity = "1";
    }
...
async function handleConfirmCheckin() {
    ...
    try {
        ...
        if (res.ok) {
            var data = await res.json();
            showToast("闁?" + (emp.name) + " Checked-In & Gift Claimed!", "success");
...
function updateCountdown() {
    ...
    let targetDate = new Date(eventTimeStr).getTime();
    if (isNaN(targetDate)) {
